...
- living-docs v2 已实现、装机(3 目标目录)并完成本仓库迁移(文档已在标准位置项目根,v1 记账产物按用户指示全部丢弃,git 历史与 tag 可回溯);待用户确认本次 sync 验收。
- 之后:套件进入试用期,按 acceptance-log.md 收集证据;连续 5 次全过则拆除 docs-acceptance 毕业。
