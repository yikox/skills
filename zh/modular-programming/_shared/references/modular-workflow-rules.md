...
默认走能保留未来可理解性的最轻量路径。对 L2/L3，默认长期产物不是独立过程 proposal，而是分支携带的 architecture patch：功能分支第一颗 commit 先把模块地图改到用户已接受的目标状态，后续实现提交让代码与它收敛。只有当复杂度或协作方式确实需要时，才使用独立可选 proposal 文档。
...
branch architecture patch commit             # 默认 L2/L3：功能分支上的已接受目标地图
architecture/changes/<date>-<change>.md      # 可选 L3 proposal，用于复杂/离线评审
architecture/adrs/ADR-<date>-<decision>.md   # 持久架构决策
architecture/modules/<module>/changes/*.md   # 可选 L2 proposal，用于复杂/离线评审
...
main 分支上的架构基线描述已经由代码实现的结构。功能分支可以短暂携带已接受的 L2/L3 目标模块地图；该分支必须在代码、验证与模块地图重新一致后才能合并。
...
| 功能、缺陷修复、重构、行为变更 | `modular-change` | L0-L3 | 按级别路径；L2/L3 默认走分支 architecture patch |
| 记录被跟踪工作的开始/进展/完成/归档 | `modular-status` | L2/L3、被跟踪的 L1 | PM 行与证据；未跟踪 L1 用简要说明 |
| 对已接受、已评审的 L2/L3 patch/proposal 做无人值守执行 | `modular-autopilot` | L2-L3 | 接入确认、决策日志、最终报告 |
| 模块化评估、重构提案、模块化设计讨论 | `modular-advisor` | - | 评估报告；分阶段重构提案；新项目设计提案 |
| 理解或讲解现有项目，通俗语言、只读 | `modular-narrator` | - | 仅对话式讲解；不产出文件 |
| 检查 architecture patch、可选 proposal、ADR、PM 行或图 | `modular-review` | - | 评审意见、修正、待解问题 |
...
- 受影响模块；
- 变更级别：`L0`、`L1`、`L2` 或 `L3`；
- 预期产物：无、PM 记录、分支 architecture patch、可选 proposal 或 ADR。
...
默认用分支第一颗 architecture patch commit 记录目标架构。不要把这颗 patch 单独合入 main；只有实现和验证让代码与模块地图一致后，分支才能合并。仅在复杂、跨天、离线或非 git 评审时，才写 `architecture/changes/<date>-<change>.md`。
...
默认用分支 architecture patch 作为目标模块地图修改。只有当模块变更需要复杂评审时，才写 `architecture/modules/<module>/changes/<date>-<change>.md`。
...
- 增加、移除或调整局部行为；
- 不改变模块边界、公共契约或核心流程；
- 无需可选 proposal 即可理解，并可在当前会话内本地验证。
...
L1 不需要 proposal。不要为常规 L1 工作新增 Active Tasks 行，除非它跨会话、携带发布/风险证据、由用户明确要求，或属于某个既有的进行中任务。当持久证据重要时，一条简洁的 Recent Updates 条目就够了。
...
- 按修复的形态而非症状的严重程度来选择级别：
  ...
  - **L2 缺陷修复**：修复需要在一个模块内做多步重构，或根因是结构性的。
    流程：常规 L2 路径；分支 architecture patch 或可选 proposal 必须包含根因分析。
...
- 结果；
- 变更的架构路径或 architecture patch commit；
- 验证结果；
...
main 上的基线架构描述已实现的当前结构。目标架构描述一个已评审、尚未落地的方向。
...
绝不要在 main 上把提案中的目标架构当作已实现基线来呈现。功能分支可以在第一颗 architecture patch commit 中携带已接受的目标架构，但必须在实现和验证让它成真后才合并。
...
- 将工作归类为 L3 并进入分支 architecture patch 路径；
- L2 branch architecture patch 摘要，在提交 patch 之前；
- L3 branch architecture patch 或 ADR 方向，在提交 patch 之前；
- 将 L2 提升为 L3；
- 产品范围取舍，或在实质不同的模块归属方案之间做选择；
- 取消进行中任务或归档大量历史。

请求 L2/L3 确认时，给出一份 3-8 条要点的决策摘要，涵盖关键变更、歧义与风险。用户应能在不打开独立 proposal 的情况下做出决定。
...
`docs-language`——PM、架构、知识、可选 proposal 文档以及决策摘要所用的语言：
...
## Architecture Patch、ADR 与计划的边界

当描述 L2/L3 变更将如何修改系统时，默认使用分支 architecture patch。它通常是功能分支第一颗 commit，修改 `architecture/main-design.md`、相关模块文档与 PM active 行到已接受的目标状态。它是由 git history 承载的过程产物，不是长期第二事实源。

只有当变更需要离线评审、在开分支前跨多个会话、涉及非 git 协作，或复杂到无法用简洁 patch 摘要表达时，才写独立可选 proposal 文档。

当记录为何在若干有意义的备选方案中选定某个持久架构方向时，使用 ADR。

当没有真正需要保留的决策时，不要写 ADR。一份有用的 ADR 至少有两个可行选项、一个会约束未来工作的决策，以及一个未来可能的读者会问“我们当初为什么选这个？”

只有在 architecture patch 或可选 proposal 被接受到足以执行之后，才使用实现计划。计划是临时的执行辅助，而非事实来源的架构。L3 计划放在 `architecture/plans/` 下，L2 计划放在 `architecture/modules/<module>/plans/` 下，并带上 `source_patch:` 或 `source_design:` 以及 `level` front matter（见 `storage-schema.md` 的 Plan Files）。一旦记录了 PM 完成，就归档或删除它们。
