...
- 同步门:push 受管分支(main-design frontmatter 的 sync_branches)前会被 pre-push hook 检查"代码动了、文档没动";被拦下时要么补文档,要么(确实无需更新时)在 commit message 加一行 `Arch-Sync: skip <module|路径 glob> <一句话理由>`(模块名放行 DRIFT,路径 glob 放行 ORPHAN)。不用 `--no-verify` 绕过。
