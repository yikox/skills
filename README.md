...
`en/modular-programming/_shared/references/modular-workflow-rules.md`（中文见 `zh/…`）的 Routing Quick Reference。
...
| `modular-advisor` | 高级：评估项目模块化程度、讨论老项目重构方案或新项目模块化设计（未接入工作流的项目也可用） | 评估报告、分阶段重构方案、模块化设计方案（只提案，不实现） |
...
仓库还包含不属于 modular-programming 套件的独立 skill，随 `install.sh` 一起安装：

| Skill | 说明 |
| --- | --- |
| `personal-style`（仅中文版） | 个人编码风格约定：模块化、接口少而简单、不留无用代码和中间产物。所有开发请求都适用。 |
...
细则见 `en/modular-programming/_shared/references/pm-maintenance-rules.md`（中文见 `zh/…`）。
...
Use $modular-advisor 评估这个项目的模块化程度，讨论重构方案。
...
套件按语言分层：`en/modular-programming/`（英文，源/主）与 `zh/modular-programming/`（中文翻译镜像，结构逐目录对等）。安装时**第一个参数选择语言**（`zh` 或 `en`，必填）。
...
缺少或非法语言参数会打印用法并以退出码 2 结束（不设隐式默认语言）。安装到目标目录的产物不含语言层——装哪个语言就是那个语言的单份。
...
en/modular-programming/    # 英文（源/主）
  ...
  modular-audit/
  modular-knowledge/
zh/modular-programming/    # 中文翻译镜像，结构与 en 逐目录对等
install.sh                 # 按语言参数（zh|en）从对应目录分发
README.md / README_EN.md   # 中文主 / 英文
```

语言镜像约定：只翻译散文；`*.py` 脚本与受控词表 token（`atomic`、`function-flow`、`uses` 等）保持英文单源，两语言目录逐字一致。

## 验证

列出当前 active skill（以 en 为例，zh 同构）：
