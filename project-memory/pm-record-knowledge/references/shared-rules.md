...
Use `PM/<project-slug>/knowledge-summary.md`. Reuse existing files from `notes-project-memory`; do not rename old project folders. If no path can be resolved safely, ask the user for the notes root.
...
- Mark commands as verified only when actually run or explicitly confirmed by the user.
- Include dates for verified commands, investigations, architecture discoveries, and decisions.
