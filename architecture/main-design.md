...
安装流:installer 用 `find zh/ -name SKILL.md` 发现技能,按目录名平铺 rsync 到各目标,再删除 deprecated 列表中的旧 skill(含 v1 的 modular-* 与 _shared)。运行流:agent 加载 living-docs-suite 的某个 skill;docs-init 在目标项目装 pre-push 同步门(调 docs-sync 自带的 check_sync.py);docs-init 与 docs-sync 结束时强制调用 docs-acceptance。legacy-v1 不参与任何流,出现变更即违反冻结契约(同步门会点名它)。
