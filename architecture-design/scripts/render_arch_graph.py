...
def load_graph(path: Path) -> Diagram:
    ...
    if raw.get("format") != "arch-graph/v0.1":
        warnings.append(f"图文件 format 不是 arch-graph/v0.1: {raw.get('format')}")
    ...
    for item in raw.get("relations", []):
        ...
        if source not in object_ids:
            warnings.append(f"关系引用了未知起点对象: {source}")
        if target not in object_ids:
            warnings.append(f"关系引用了未知终点对象: {target}")
        relations.append(
            Relation(
                source=source,
                target=target,
...
def group_fragment_bounds(
    ...
    for child_id in contained:
        _, y = positions[child_id]
        rows.setdefault(normalize_route_coord(y), []).append(child_id)

    fragments: List[BBox] = []
    contiguous_threshold = CELL_WIDTH * 1.1
    for _, row_child_ids in sorted(rows.items()):
        row_child_ids.sort(key=lambda child_id: positions[child_id][0])
        run: List[str] = []
        previous_x: Optional[float] = None
        for child_id in row_child_ids:
            x, _ = positions[child_id]
            if previous_x is not None and x - previous_x > contiguous_threshold and run:
                fragments.append(object_run_bounds(run, positions, objects_by_id))
                run = []
            run.append(child_id)
            previous_x = x
        if run:
    ...
    return sorted(fragments, key=lambda box: (box[1], box[0]))
...
def group_bounds(
    ...
    fragments = group_fragment_bounds(group, positions, objects_by_id)
    if not fragments:
        return None
    return (
        min(box[0] for box in fragments),
        min(box[1] for box in fragments),
        max(box[2] for box in fragments),
        max(box[3] for box in fragments),
    )
...
def render_group(
    ...
    tooltip += "</title>"
    shape_svg = "\n".join(
        f'<rect x="{left:.1f}" y="{top:.1f}" width="{right - left:.1f}" height="{bottom - top:.1f}" rx="10" class="group-shape" />'
...
def relation_port_pairs(
    ...
    source_center = positions[relation.source]
    target_center = positions[relation.target]
    source = objects_by_id[relation.source]
    target = objects_by_id[relation.target]
    sx, sy = source_center
    tx, ty = target_center
    dx = tx - sx
    dy = ty - sy
    ...
    for source_side, target_side in preferred_sides:
        pair = (
            normalize_route_point(object_port(source, source_center, source_side)),
            normalize_route_point(object_port(target, target_center, target_side)),
        )
        if pair in seen:
            continue
        seen.add(pair)
...
def orthogonal_search_route_candidates(
    ...
    hard_obstacles = [
        ...
        if object_id not in {relation.source, relation.target}
    ]
    soft_obstacles = group_label_boxes
    candidates: List[List[Point]] = []
    for start, end in relation_port_pairs(relation, positions, objects_by_id):
...
def relation_route_candidates(
    ...
    start_h = object_port(source, source_center, h_source_side)
    end_h = object_port(target, target_center, h_target_side)
    start_v = object_port(source, source_center, v_source_side)
    end_v = object_port(target, target_center, v_target_side)
    ...
    for offset in [-78, 78, -126, 126, -178, 178]:
        ...
        start = object_port(source, source_center, source_side)
        end = object_port(target, target_center, target_side)
    ...
    for lane_y, side in [(outer_top, "top"), (outer_bottom, "bottom")]:
        start = object_port(source, source_center, side)
        end = object_port(target, target_center, side)
        add([start, (start[0], lane_y), (end[0], lane_y), end])
...
def route_relation(
    ...
    candidates = relation_route_candidates(relation, positions, objects_by_id, all_boxes)
    candidates.extend(
        orthogonal_search_route_candidates(
            relation,
            positions,
            objects_by_id,
            object_boxes_by_id,
    ...
    candidates = [
        route_with_endpoint_stubs(candidate, positions[relation.source], positions[relation.target])
...
def render_routed_relation_line(routed: RoutedRelation) -> str:
    return "\n".join(
        [
            ...
            f'<path d="{path_data(routed.points)}" class="relation-line"{routed.dash} marker-end="url(#arrow)" />',
            "</g>",
        ]
    )
...
def render_routed_relation_hover(routed: RoutedRelation) -> str:
    ...
    return "\n".join(
        [
            ...
            f'<path d="{path_data(routed.points)}" class="relation-hover-line"{routed.dash} />',
            bubble_svg,
            "</g>",
        ]
    )
...
def render_svg(diagram: Diagram, link_base: Optional[Path] = None) -> str:
    ...
    group_fragments_by_id = {
        group.group_id: group_fragment_bounds(group, positions, objects_by_id)
        for group in diagram.groups
    }
    group_bounds_values = [bounds for fragments in group_fragments_by_id.values() for bounds in fragments]
    object_boxes_by_id = {
    ...
    group_label_boxes = [
        padded_box(group_label_box(fragments[0]), 6)
        for fragments in group_fragments_by_id.values()
        if fragments
    ]
    placed_segments: List[Segment] = []
